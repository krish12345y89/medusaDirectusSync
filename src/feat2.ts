...
const syncMedusaToDirectus = async (medusaProduct) => {
  try {
    ...
    const createdDirectusProduct = await directus.request(
      createItem("product", directusData)
    );
...
const syncMedusaUpdateToDirectus = async (medusaProduct) => {
 ...
 const updatedDirectusData = {
    ...
    const updatedDirectusProduct = await directus.request(
      updateItem("product", existingSync.directusId, updatedDirectusData)
    );
